...
export function AdminComplaintList({ filters, refreshTrigger }: AdminComplaintListProps) {
      ...
      {complaints.map((complaint) => (
        <Card key={complaint.id}>
          ...
          <CardContent className="pt-0">
            <p className="text-sm text-muted-foreground mb-4 line-clamp-3">{complaint.description}</p>
            ...
            {updatingId === complaint.id && (
              <div className="text-xs text-muted-foreground mt-2">Updating status...</div>
            )}
          </CardContent>
        </Card>
      ))}
